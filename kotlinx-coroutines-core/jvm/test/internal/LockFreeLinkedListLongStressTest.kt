...
/**
 * This stress test has 2 threads adding on one side on list, 2 more threads adding on the other, and 6 threads iterating and concurrently
 * removing items. The resulting list that is being stressed is long.
 */
class LockFreeLinkedListLongStressTest : TestBase() {
    ...
    fun testStress() {
        ...
        val expected = iterator {
            for (i in 0 until nAdded) if (!shallRemove(i)) yield(i)
        }
        list.forEach { node ->
