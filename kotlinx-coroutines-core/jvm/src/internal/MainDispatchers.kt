...
/**
 * If anything goes wrong while trying to create main dispatcher (class not found, initialization failed, etc), then replace the main
 * dispatcher with a special stub that throws an error message on any attempt to actually use it.
 *
...
private class MissingMainCoroutineDispatcher(
    ...
    override fun limitedParallelism(parallelism: Int, name: String?): CoroutineDispatcher = missing()

    override fun invokeOnTimeout(timeMillis: Long, block: Runnable, context: CoroutineContext): DisposableHandle = missing()

    override fun dispatch(context: CoroutineContext, block: Runnable) = missing()

    override fun scheduleResumeAfterDelay(timeMillis: Long, continuation: CancellableContinuation<Unit>) = missing()
