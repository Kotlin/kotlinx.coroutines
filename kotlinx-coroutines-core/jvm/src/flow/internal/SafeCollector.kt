...
/**
 * A safe collector is an instance of [FlowCollector] that ensures that neither context preservation nor exception transparency invariants
 * are broken. Instances of [SafeCollector] are used in flow operators that provide raw access to the [FlowCollector] e.g. [Flow.transform].
 * Mechanically, each [emit] call captures [currentCoroutineContext], ensures it is not different from the previously caught one and
 * proceeds further. If an exception is thrown from the downstream, it is caught, and any further attempts to [emit] lead to the
 * [IllegalStateException].
 *
 * ### Performance hacks
 *
 * Implementor of [ContinuationImpl] (that will be preserved as ABI nearly forever) in order to properly control `intercepted()` lifecycle.
 * The safe collector implements [ContinuationImpl] to pretend it *is* a state-machine of its own `emit` method. It is [ContinuationImpl]
 * and not any other [Continuation] subclass because only [ContinuationImpl] supports `intercepted()` caching. This is the most
 * performance-sensitive place in the overall flow pipeline, because otherwise safe collector is forced to allocate a state machine on each
 * element being emitted for each intermediate stage where the safe collector is present.
 *
...
actual constructor(
     ...
     *
     * First it checks that it is not used concurrently (which we explicitly prohibit), and then just caches an instance of the completion_
     * in order to avoid extra allocation on each emit, making it effectively garbage-free on its hot-path.
     *
     * See `emit` overload.
     */
     ...
     * - Having a non-intercepted, non-cancellable caller's `uCont`, we leverage our implementation knowledge and invoke `collector.emit(T)`
     *   as `collector.emit(value: T, completion: Continuation)`, passing `this` as the completion. We also setup `this` state, so if the
     *   `completion.resume` is invoked, we are invoking `uCont.resume` properly in accordance with
     *   `ContinuationImpl`/`BaseContinuationImpl` internal invariants.
     *
