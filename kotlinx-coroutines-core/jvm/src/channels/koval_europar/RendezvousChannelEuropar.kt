...
//        if (res != null) return res
    ...
    private suspend fun <T> sendOrReceiveSuspendFast(element: Any?): T = suspendCoroutineUninterceptedOrReturn { uCont ->
        ...
        try_again@ while (true) { // CAS loop
            ...
            // Check if the waiting queue is empty.
            if (deqIdx == enqIdx) {
                if (cont == null) cont = CancellableContinuationImpl(uCont.intercepted(), resumeMode = MODE_CANCELLABLE)
    ...
    public val onSendEuropar: Param1RegInfo<Unit>
        get() = Param1RegInfo<Unit>(this, RendezvousChannelEuropar<*>::regSelectSend, Companion::actOnSendAndOnReceive)
    public val onReceiveEuropar: Param0RegInfo<E>
        get() = Param0RegInfo<E>(this, RendezvousChannelEuropar<*>::regSelectReceive, Companion::actOnSendAndOnReceive)
