 ...
 *
 * In debug mode every coroutine is assigned a unique consecutive identifier. Every thread that executes a coroutine has its name modified
 * to include the name and identifier of the currently running coroutine.
 *
 ...
 *
 * Debugging facilities are implemented by [newCoroutineContext][CoroutineScope.newCoroutineContext] function that is used in all coroutine
 * builders to create context of a new coroutine.
 */
public const val DEBUG_PROPERTY_NAME: String = "kotlinx.coroutines.debug"

/**
 * Name of the boolean property that controls stacktrace recovery (enabled by default) on JVM. Stacktrace recovery is enabled if both debug
 * and stacktrace recovery modes are enabled.
 *
 * Stacktrace recovery mode wraps every exception into the exception of the same type with original exception as cause, but with stacktrace
 * of the current coroutine. Exception is instantiated using reflection by using no-arg, cause or cause and message constructor.
 *
 * This mechanism is currently supported for channels, [async], [launch], [coroutineScope], [supervisorScope] and [withContext] builders.
 */
