...
/**
 * Coerces the given [Duration] to a millisecond delay. Negative values are coerced to zero, values that cannot be represented in
 * milliseconds as long ("infinite" duration) are coerced to [Long.MAX_VALUE] and durations lesser than a millisecond are coerced to 1
 * millisecond.
 *
 * The rationale of coercion:
 * 1) Too large durations typically indicate infinity and Long.MAX_VALUE is the best approximation of infinity we can provide.
...
private fun Duration.coerceToMillis(): Long {
    ...
    return if (seconds < maxSeconds || seconds == maxSeconds && nano < maxNanos) toMillis() else Long.MAX_VALUE
}
