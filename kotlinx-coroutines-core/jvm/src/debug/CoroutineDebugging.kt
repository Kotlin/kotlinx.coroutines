 ...
 *    documentation. The usual way to enable the debug mode is with the [kotlinx.coroutines.DEBUG_PROPERTY_NAME] system property.
...
internal class ArtificialStackFrames {
     ...
     *
     * Appearance of this function in stack traces does not mean that it was called. Instead, it is used as a marker that separates the part
     * of the stack trace with the code executed in a coroutine from the stack trace of the code that launched the coroutine.
     *
     * In earlier versions of kotlinx-coroutines, this was displayed as "(Coroutine creation stacktrace)", which caused problems for tooling
     * that processes stack traces: https://github.com/Kotlin/kotlinx.coroutines/issues/2291
     *
     ...
     *
     * Appearance of this function in stack traces does not mean that it was called. Instead, when one coroutine invokes another, this is
     * used as a marker in the stack trace to denote where the execution of one coroutine ends and that of another begins.
     *
     * In earlier versions of kotlinx-coroutines, this was displayed as "(Coroutine boundary)", which caused problems for tooling that
     * processes stack traces: https://github.com/Kotlin/kotlinx.coroutines/issues/2291
     */
 ...
 * 1. The package name, it seems, is needed for the IDE to detect stack trace elements reliably. It is `_COROUTINE` since this is a valid
 *    identifier.
