...
class PromiseTestWeb : TestBase() {
    ...
    fun testPromiseRejectedAsDeferred() = GlobalScope.promise {
        ...
        val promise =
            Promise<JsAny?> { _, reject ->
                promiseReject = reject
            }
        val deferred = promise.asDeferred()
        // reject after converting to deferred to avoid "Unhandled promise rejection" warnings
        @Suppress("CAST_NEVER_SUCCEEDS") promiseReject(TestException("Rejected").toJsReference() as JsPromiseError)
        try {
            deferred.await()
            expectUnreached()
        } catch (e: Throwable) {
    ...
    fun testAwaitPromiseRejectedWithKotlinException() = GlobalScope.promise {
        ...
        @Suppress("CAST_NEVER_SUCCEEDS") r(RuntimeException("Rejected").toJsReference() as JsPromiseError)
...
@OptIn(ExperimentalWasmJsInterop::class) private fun jsPromiseRejectedWithString(): Promise<JsBigInt> = js("Promise.reject(\"Rejected\")")
