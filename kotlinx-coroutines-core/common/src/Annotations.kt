...
/**
 * Marks declarations in the coroutines that are **delicate** &mdash; they have limited use-case and shall be used with care in general
 * code. Any use of a delicate declaration has to be carefully reviewed to make sure it is properly used and does not create problems like
 * memory and resource leaks. Carefully read documentation of any declaration marked as `DelicateCoroutinesApi`.
 */
@MustBeDocumented
...
/**
 * Marks declarations that are still **experimental** in coroutines API, which means that the design of the corresponding declarations has
 * open issues which may (or may not) lead to their changes in the future. Roughly speaking, there is a chance that those declarations will
 * be deprecated in the near future or the semantics of their behavior may change in some way that may break some code.
 */
@MustBeDocumented
@Retention(value = AnnotationRetention.BINARY)
@Target(
 ...
 *
 * Flow preview has **no** backward compatibility guarantees, including both binary and source compatibility. Its API and semantics can and
 * will be changed in next releases.
 *
 * Feature preview can be used to evaluate its real-world strengths and weaknesses, gather and provide feedback. According to the feedback,
 * [Flow] will be refined on its road to stabilization and promotion to a stable API.
 *
...
/**
 * Marks declarations that are **obsolete** in coroutines API, which means that the design of the corresponding declarations has serious
 * known flaws and they will be redesigned in the future. Roughly speaking, these declarations will be deprecated in the future but there is
 * no replacement for them yet, so they cannot be deprecated right away.
 */
@MustBeDocumented
@Retention(value = AnnotationRetention.BINARY)
@RequiresOptIn(level = RequiresOptIn.Level.WARNING)
...
/**
 * Marks declarations that are **internal** in coroutines API, which means that should not be used outside of `kotlinx.coroutines`, because
 * their signatures and semantics will change between future releases without any warnings and without providing any migration aids.
 */
@MustBeDocumented
@Retention(value = AnnotationRetention.BINARY)
@Target(AnnotationTarget.CLASS, AnnotationTarget.FUNCTION, AnnotationTarget.TYPEALIAS, AnnotationTarget.PROPERTY)
