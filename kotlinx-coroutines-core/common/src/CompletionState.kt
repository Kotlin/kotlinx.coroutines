...
internal fun <T> Result<T>.toState(caller: CancellableContinuation<*>): Any? = getOrElse {
...
internal fun <T> recoverResult(state: Any?, uCont: Continuation<T>): Result<T> =
    if (state is CompletedExceptionally) Result.failure(recoverStackTrace(state.cause, uCont)) else Result.success(state as T)

/**
 * Class for an internal state of a job that was cancelled (completed exceptionally).
 *
 * @param cause the exceptional completion cause. It's either original exceptional cause or artificial [CancellationException] if no cause
 *   was provided
 */
 ...
 * @param cause the exceptional completion cause. If `cause` is null, then a [CancellationException] if created on first access to [cause]
 *   property.
 */
