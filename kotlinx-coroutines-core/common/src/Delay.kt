...
/**
 * This dispatcher _feature_ is implemented by [CoroutineDispatcher] implementations that natively support scheduled execution of tasks.
 *
 * Implementation of this interface affects operation of [delay][kotlinx.coroutines.delay] and [withTimeout] functions.
 *
...
public interface Delay {
     ...
     *
     * Continuation **must be scheduled** to resume even if it is already cancelled, because a cancellation is just an exception that the
     * coroutine that used `delay` might wanted to catch and process. It might need to close some resources in its `finally` blocks, for
     * example.
     *
     * This implementation is supposed to use dispatcher's native ability for scheduled execution in its thread(s). In order to avoid an
     * extra delay of execution, the following code shall be used to resume this [continuation] when the code is already executing in the
     * appropriate thread:
     * ```kotlin
    ...
    /**
     * Schedules invocation of a specified [block] after a specified delay [timeMillis]. The resulting [DisposableHandle] can be used to
     * [dispose][DisposableHandle.dispose] of this invocation request if it is not needed anymore.
     */
...
/**
 * Enhanced [Delay] interface that provides additional diagnostics for [withTimeout]. Is going to be removed once there is proper
 * JVM-default support. Then we'll be able put this function into [Delay] without breaking binary compatibility.
 */
@InternalCoroutinesApi
...
/**
 * Delays coroutine for at least the given time without blocking a thread and resumes it after a specified time. If the given [timeMillis]
 * is non-positive, this function returns immediately.
 *
...
/**
 * Delays coroutine for at least the given [duration] without blocking a thread and resumes it after the specified time. If the given
 * [duration] is non-positive, this function returns immediately.
 *
 * This suspending function is cancellable: if the [Job] of the current coroutine is cancelled while this suspending function is waiting,
 * this function immediately resumes with [CancellationException]. There is a **prompt cancellation guarantee**: even if this function is
 * ready to return the result, but was cancelled while suspended, [CancellationException] will be thrown. See [suspendCancellableCoroutine]
