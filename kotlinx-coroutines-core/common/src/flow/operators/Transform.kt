...
/**
 * Folds the given flow with [operation], emitting every intermediate result, including [initial] value. Note that initial value should be
 * immutable (or should not be mutated) as it is shared between different collectors. For example:
 * ```
...
/**
 * Reduces the given flow with [operation], emitting every intermediate result, including initial value. The first element is taken as
 * initial value for operation accumulator. This operator has a sibling with initial value -- [scan].
 *
 * For example:
 * ```
...
/**
 * Splits the given flow into a flow of non-overlapping lists each not exceeding the given [size] but never empty. The last emitted list may
