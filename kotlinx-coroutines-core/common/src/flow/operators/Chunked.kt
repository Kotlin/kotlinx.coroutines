...
public fun <T> Flow<T>.chunkedNonResetableTimer(
    durationMs: Long,
    fixedIntervals: Boolean = false,
    minSize: Int = 0,
    maxSize: Int = NO_MAXIMUM,
): Flow<List<T>> {
    require(minSize >= 0)
    require(maxSize == NO_MAXIMUM || maxSize >= minSize)
    return scopedFlow { downstream ->
        val chunker: Chunker<T> =
            if (fixedIntervals) EagerChunker(durationMs, minSize, maxSize)
            else LazyChunker(durationMs, minSize, maxSize)

        launch {
            collect { value -> chunker.send(value) }
            chunker.close()
        }
        downstream.emitAll(chunker.chunks)
    }
}
...
private sealed class Message<out T> {
    class NewElement<out T>(val value: T) : Message<T>()
    object TimeIsUp : Message<Nothing>()
    object Stop : Message<Nothing>()
}

private abstract class Chunker<T> {
    protected val inbox = Channel<Message<T>>(Channel.CHANNEL_DEFAULT_CAPACITY)

    abstract val chunks: Flow<List<T>>

    suspend fun send(value: T) {
        inbox.send(Message.NewElement(value))
    }
...
private class LazyChunker<T>(
    ...
    override val chunks = flow {
        coroutineScope {
            ...
            for (message in inbox) {
                when (message) {
                    is Message.NewElement -> {
                        if (accumulator.isEmpty()) timer = startTimer()
                        accumulator.add(message.value)
                        if (accumulator.size == maxSize) {
                            timer?.cancelAndJoin()
                            emit(accumulator.drain())
                        }
                    }

                    Message.TimeIsUp -> {
                        if (accumulator.size >= minSize) {
                            emit(accumulator.drain())
                        }
                    }
            ...
            if (accumulator.size >= max(1, minSize)) emit(accumulator)
        }
    }

    private fun CoroutineScope.startTimer() = launch {
        delay(durationMs)
        inbox.send(Message.TimeIsUp)
    }
}

private class EagerChunker<T>(
    private val durationMs: Long,
    private val minSize: Int,
    private val maxSize: Int
) : Chunker<T>() {

    override val chunks: Flow<List<T>> = flow {
        coroutineScope {
            val accumulator = ArrayList<T>()
