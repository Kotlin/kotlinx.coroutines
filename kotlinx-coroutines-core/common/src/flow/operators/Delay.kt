...
public fun <T> Flow<T>.debounce(timeout: Duration): Flow<T> {
    require(!timeout.isNegative()) { "Debounce timeout should not be negative" }
    if (timeout == Duration.ZERO) return this
    return debounceInternal { timeout }
}
...
public fun <T> Flow<T>.debounce(timeoutMillis: (T) -> Long): Flow<T> =
    debounceInternal { emittedItem -> timeoutMillis(emittedItem).milliseconds }

private fun <T> Flow<T>.debounceInternal(timeoutSelector: (T) -> Duration): Flow<T> =
    scopedFlow { downstream ->
        ...
        while (lastValue !== DONE) {
            var timeout = Duration.ZERO // will be always computed when lastValue != null
            // Compute timeout for this value
            if (lastValue != null) {
                timeout = timeoutSelector(NULL.unbox(lastValue))
                require(!timeout.isNegative()) { "Debounce timeout should not be negative" }
            ...
            assert { lastValue == null || timeout.isPositive() }
...
public fun <T> Flow<T>.sample(period: Duration): Flow<T> {
    require(period.isPositive()) { "Sample period should be positive" }
    return scopedFlow { downstream ->
        val values = produce(capacity = Channel.CONFLATED) {
            collect { value -> send(value ?: NULL) }
        }
        var lastValue: Any? = null
        val ticker = fixedPeriodTicker(period)
        while (lastValue !== DONE) {
            select<Unit> {
 ...
 */
@FlowPreview
public fun <T> Flow<T>.sample(periodMillis: Long): Flow<T> = sample(periodMillis.milliseconds)
