 ...
 * subscribers. See the [SharedFlow] documentation for the general concepts of shared flows.
 *
 * The starting of the sharing coroutine is controlled by the [started] parameter. The following options are supported.
 *
 * - [Eagerly][SharingStarted.Eagerly] &mdash; the upstream flow is started even before the first subscriber appears. Note that in this case
 *   all values emitted by the upstream beyond the most recent values as specified by [replay] parameter **will be immediately discarded**.
 * - [Lazily][SharingStarted.Lazily] &mdash; starts the upstream flow after the first subscriber appears, which guarantees that this first
 *   subscriber gets all the emitted values, while subsequent subscribers are only guaranteed to get the most recent [replay] values. The
 *   upstream flow continues to be active even when all subscribers disappear, but only the most recent [replay] values are cached without
 *   subscribers.
 * - [WhileSubscribed()][SharingStarted.WhileSubscribed] &mdash; starts the upstream flow when the first subscriber appears, immediately
 *   stops when the last subscriber disappears, keeping the replay cache forever. It has additional optional configuration parameters as
 *   explained in its documentation.
 * - A custom strategy can be supplied by implementing the [SharingStarted] interface.
 *
 * The `shareIn` operator is useful in situations when there is a _cold_ flow that is expensive to create and/or to maintain, but there are
 * multiple subscribers that need to collect its values. For example, consider a flow of messages coming from a backend over the expensive
 * network connection, taking a lot of time to establish. Conceptually, it might be implemented like this:
 * ```
 ...
 * ```
 *
 * If this flow is directly used in the application, then every time it is collected a fresh connection is established, and it will take a
 * while before messages start flowing. However, we can share a single connection and establish it eagerly like this:
 * ```
 * val messages: SharedFlow<Message> = backendMessages.shareIn(scope, SharingStarted.Eagerly)
 * ```
 *
 * Now a single connection is shared between all collectors from `messages`, and there is a chance that the connection is already
 * established by the time it is needed.
 *
 * ### Upstream completion and error handling
 *
 * **Normal completion of the upstream flow has no effect on subscribers**, and the sharing coroutine continues to run. If a strategy like
 * [SharingStarted.WhileSubscribed] is used, then the upstream can get restarted again. If a special action on upstream completion is
 * needed, then an [onCompletion] operator can be used before the `shareIn` operator to emit a special value in this case, like this:
 * ```
 * backendMessages
 ...
 * ```
 *
 * Any exception in the upstream flow terminates the sharing coroutine without affecting any of the subscribers, and will be handled by the
 * [scope] in which the sharing coroutine is launched. Custom exception handling can be configured by using the [catch] or [retry] operators
 * before the `shareIn` operator. For example, to retry connection on any `IOException` with 1 second delay between attempts, use:
 * ```
 ...
 * ### Initial value
 *
 * When a special initial value is needed to signal to subscribers that the upstream is still loading the data, use the [onStart] operator
 * on the upstream flow. For example:
 * ```
 * backendMessages
 ...
 *
 * The `shareIn` operator runs the upstream flow in a separate coroutine, and buffers emissions from upstream as explained in the [buffer]
 * operator's description, using a buffer of [replay] size or the default (whichever is larger). This default buffering can be overridden
 * with an explicit buffer configuration by preceding the `shareIn` call with [buffer] or [conflate], for example:
 *
 * - `buffer(0).shareIn(scope, started, 0)` &mdash; overrides the default buffer size and creates a [SharedFlow] without a buffer.
 *   Effectively, it configures sequential processing between the upstream emitter and subscribers, as the emitter is suspended until all
 *   subscribers process the value. Note, that the value is still immediately discarded when there are no subscribers.
 * - `buffer(b).shareIn(scope, started, r)` &mdash; creates a [SharedFlow] with `replay = r` and `extraBufferCapacity = b`.
 * - `conflate().shareIn(scope, started, r)` &mdash; creates a [SharedFlow] with `replay = r`, `onBufferOverflow = DROP_OLDEST`, and
 *   `extraBufferCapacity = 1` when `replay == 0` to support this strategy.
 *
 * ### Operator fusion
 *
 * Application of [flowOn][Flow.flowOn], [buffer] with [RENDEZVOUS][Channel.RENDEZVOUS] capacity, or [cancellable] operators to the
 * resulting shared flow has no effect.
 *
 * ### Exceptions
 *
    ...
    return launch(context, start = start) { // the single coroutine to rule the sharing
        ...
        when {
            ...
            started === SharingStarted.Lazily -> {
                ...
                started
                    .command(shared.subscriptionCount)
                    ...
                    .collectLatest { // cancels block on new emission
                        when (it) {
                            ...
                            SharingCommand.STOP -> {
                                /* just cancel and do nothing else */
                            }
 ...
 * general concepts of state flows.
 *
 ...
 *
 * The `stateIn` operator is useful in situations when there is a _cold_ flow that provides updates to the value of some state and is
 * expensive to create and/or to maintain, but there are multiple subscribers that need to collect the most recent state value. For example,
 * consider a flow of state updates coming from a backend over the expensive network connection, taking a lot of time to establish.
 * Conceptually it might be implemented like this:
 * ```
 ...
 *
 * If this flow is directly used in the application, then every time it is collected a fresh connection is established, and it will take a
 * while before state updates start flowing. However, we can share a single connection and establish it eagerly like this:
 * ```
 * val state: StateFlow<State> = backendMessages.stateIn(scope, SharingStarted.Eagerly, State.LOADING)
 * ```
 *
 * Now, a single connection is shared between all collectors from `state`, and there is a chance that the connection is already established
 * by the time it is needed.
 *
 * ### Upstream completion and error handling
 *
 * **Normal completion of the upstream flow has no effect on subscribers**, and the sharing coroutine continues to run. If a a strategy like
 * [SharingStarted.WhileSubscribed] is used, then the upstream can get restarted again. If a special action on upstream completion is
 * needed, then an [onCompletion] operator can be used before the `stateIn` operator to emit a special value in this case. See the [shareIn]
 * operator's documentation for an example.
 *
 * Any exception in the upstream flow terminates the sharing coroutine without affecting any of the subscribers, and will be handled by the
 * [scope] in which the sharing coroutine is launched. Custom exception handling can be configured by using the [catch] or [retry] operators
 * before the `stateIn` operator, similarly to the [shareIn] operator.
 *
 * ### Operator fusion
 *
 * Application of [flowOn][Flow.flowOn], [conflate][Flow.conflate], [buffer] with [CONFLATED][Channel.CONFLATED] or
 * [RENDEZVOUS][Channel.RENDEZVOUS] capacity, [distinctUntilChanged][Flow.distinctUntilChanged], or [cancellable] operators to a state flow
 * has no effect.
 *
 * @param scope the coroutine scope in which sharing is started.
 * @param started the strategy that controls when sharing is started and stopped.
 * @param initialValue the initial value of the state flow. This value is also used when the state flow is reset using the
 *   [SharingStarted.WhileSubscribed] strategy with the `replayExpirationMillis` parameter.
 */
...
/**
 * Starts the upstream flow in a given [scope], suspends until the first value is emitted, and returns a _hot_ [StateFlow] of future
 * emissions, sharing the most recently emitted value from this running instance of the upstream flow with multiple downstream subscribers.
 ...
 *
 * Unlike simple upcasting, the returned flow prevents casting back to the original hot flow type, ensuring that implementation details
 * remain encapsulated.
 *
 * Example:
 * ```
 ...
 *
 * The [action] is called before any value is emitted from the upstream flow to this subscription but after the subscription is established.
 * It is guaranteed that all emissions to the upstream flow that happen inside or immediately after this `onSubscription` action will be
 * collected by this subscription.
 *
 * The receiver of the [action] is [FlowCollector], so `onSubscription` can emit additional elements.
 */
public fun <T> SharedFlow<T>.onSubscription(action: suspend FlowCollector<T>.() -> Unit): SharedFlow<T> = SubscribedSharedFlow(this, action)
...
private class SubscribedSharedFlow<T>(
    ...
    override suspend fun collect(collector: FlowCollector<T>) = sharedFlow.collect(SubscribedFlowCollector(collector, action))
}

/**
 * Returns a flow that invokes the given [action] **after** this state flow starts to be collected (after the subscription is registered).
 *
 * The [action] is called before any value is emitted from the upstream flow to this subscription but after the subscription is established.
 * It is guaranteed that all emissions to the upstream flow that happen inside or immediately after this `onSubscription` action will be
 * collected by this subscription.
 ...
 * The receiver of the [action] is [FlowCollector], so `onSubscription` can emit additional elements.
 */
public fun <T> StateFlow<T>.onSubscription(action: suspend FlowCollector<T>.() -> Unit): StateFlow<T> = SubscribedStateFlow(this, action)
...
private class SubscribedStateFlow<T>(
    ...
    override suspend fun collect(collector: FlowCollector<T>) = stateFlow.collect(SubscribedFlowCollector(collector, action))
}
