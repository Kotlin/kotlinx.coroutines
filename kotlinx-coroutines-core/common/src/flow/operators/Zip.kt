 ...
 *
 * The receiver of the [transform] is [FlowCollector] and thus `transform` is a generic function that may transform emitted element, skip it
 * or emit it multiple times.
 *
...
public fun <T1, T2, R> Flow<T1>.combineTransform(
    ...
    combineTransformUnsafe(this, flow) { args: Array<*> ->
        transform(
            args[0] as T1,
            args[1] as T2,
        )
...
public fun <T1, T2, R> combineTransform(
    ...
    combineTransformUnsafe(flow, flow2) { args: Array<*> ->
        transform(
            args[0] as T1,
            args[1] as T2,
        )
...
public fun <T1, T2, T3, R> combineTransform(
    ...
    combineTransformUnsafe(flow, flow2, flow3) { args: Array<*> ->
        transform(
            args[0] as T1,
            args[1] as T2,
            args[2] as T3,
...
public fun <T1, T2, T3, T4, R> combine(
    ...
    combineUnsafe(flow, flow2, flow3, flow4) { args: Array<*> ->
        transform(
            args[0] as T1,
            args[1] as T2,
            args[2] as T3,
...
public fun <T1, T2, T3, T4, R> combineTransform(
    ...
    combineTransformUnsafe(flow, flow2, flow3, flow4) { args: Array<*> ->
        transform(
            args[0] as T1,
            args[1] as T2,
            args[2] as T3,
...
public fun <T1, T2, T3, T4, T5, R> combine(
    ...
    combineUnsafe(flow, flow2, flow3, flow4, flow5) { args: Array<*> ->
        transform(
            args[0] as T1,
            args[1] as T2,
            args[2] as T3,
...
public fun <T1, T2, T3, T4, T5, R> combineTransform(
    ...
    combineTransformUnsafe(flow, flow2, flow3, flow4, flow5) { args: Array<*> ->
        transform(
            args[0] as T1,
            args[1] as T2,
            args[2] as T3,
...
/**
 * Zips values from the current flow (`this`) with [other] flow using provided [transform] function applied to each pair of values. The
 * resulting flow completes as soon as one of the flows completes and cancel is called on the remaining flow.
 *
 * It can be demonstrated with the following example:
 * ```
 ...
 * ### Buffering
 *
 * The upstream flow is collected sequentially in the same coroutine without any buffering, while the [other] flow is collected concurrently
 * as if `buffer(0)` is used. See documentation in the [buffer] operator for explanation. You can use additional calls to the [buffer]
 * operator as needed for more concurrency.
 */
