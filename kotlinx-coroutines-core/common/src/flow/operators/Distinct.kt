 ...
 *
 * Note that any instance of [StateFlow] already behaves as if `distinctUntilChanged` operator is applied to it, so applying
...
/**
 * Returns flow where all subsequent repetitions of the same key are filtered out, where keys are extracted with [keySelector] function and
 * compared with each other via the provided [areEquivalent] function.
 *
