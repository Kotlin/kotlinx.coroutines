...
/**
 * A terminal operator that returns `true` if all elements match the given [predicate], or returns `false` and cancels the flow as soon as
 * the first element not matching the predicate is encountered.
 *
 * If the flow terminates without emitting any elements, the function returns `true` because there are no elements in it that *do not* match
 * the predicate. See a more detailed explanation of this logic concept in the
...
/**
 * A terminal operator that returns `true` if no elements match the given [predicate], or returns `false` and cancels the flow as soon as
 * the first element matching the predicate is encountered.
 *
 * If the flow terminates without emitting any elements, the function returns `true` because there are no elements in it that match the
 * predicate. See a more detailed explanation of this logic concept in the ["Vacuous truth"](https://en.wikipedia.org/wiki/Vacuous_truth)
 * article.
 *
