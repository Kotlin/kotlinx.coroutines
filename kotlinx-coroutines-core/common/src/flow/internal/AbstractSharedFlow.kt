 ...
 *
 * It is exposed as a regular [StateFlow] in our public API, but it is implemented as [SharedFlow] undercover to avoid conflations of
 * consecutive updates because the subscription count is very sensitive to it.
 *
 ...
 * ```
 *
 * If the flow is shared within the same dispatcher (e.g. Main) or with a slow/throttled one, the `SharingStarted.Lazily` will never be able
 * to start the source: `first` sees the initial value and immediately unsubscribes, leaving the asynchronous `SharingStarted` with
 * conflated zero.
 *
