 ...
 *
 * **DO NOT CHANGE THE CONSTANT VALUE**. It might be inlined into legacy user code that was calling inline
 * `suspendAtomicCancellableCoroutine` function and did not support reuse.
 */
internal const val MODE_ATOMIC = 0

/**
 * Cancellable dispatch mode. It is used by user-facing [suspendCancellableCoroutine]. Note, that implementation of cancellability checks
 * mode via [Int.isCancellableMode] extension.
 *
 ...
 * [Int.isCancellableMode] extension; implementation of reuse checks mode via [Int.isReusableMode] extension.
 */
internal const val MODE_CANCELLABLE_REUSABLE = 2

/**
 * Undispatched mode for [CancellableContinuation.resumeUndispatched]. It is used when the thread is right, but it needs to be marked with
 * the current coroutine.
 */
internal const val MODE_UNDISPATCHED = 4

/**
 * Initial mode for [DispatchedContinuation] implementation, should never be used for dispatch, because it is always overwritten when
 * continuation is resumed with the actual resume mode.
 */
...
internal abstract class DispatchedTask<in T> internal constructor(@JvmField var resumeMode: Int) : SchedulerTask() {
    ...
    /**
     * There are two implementations of `DispatchedTask`:
     * - [DispatchedContinuation] is just an intermediate storage that stores the exception that has its stack-trace properly recovered and
     *   is ready to pass to the [delegate] continuation directly.
     * - [CancellableContinuationImpl] stores raw cause of the failure in its state; when it needs to be dispatched its stack-trace has to
     *   be recovered, so it overrides this method for that purpose.
     */
    internal open fun getExceptionalResult(state: Any?): Throwable? = (state as? CompletedExceptionally)?.cause
    ...
    /**
     * Machinery that handles fatal exceptions in kotlinx.coroutines. There are two kinds of fatal exceptions:
     *
     * 1) Exceptions from kotlinx.coroutines code. Such exceptions indicate that either the library or the compiler has a bug that breaks
     *    internal invariants. They usually have specific workarounds, but require careful study of the cause and should be reported to the
     *    maintainers and fixed on the library's side anyway.
     *
     * 2) Exceptions from `ThreadContextElement.updateThreadContext` and `ThreadContextElement.restoreThreadContext`. While a user code can
     *    trigger such exception by providing an improper implementation of `ThreadContextElement`, we can't ignore it because it may leave
     *    coroutine in the inconsistent state. If you encounter such exception, you can either disable this context element or wrap it into
     *    another context element that catches all exceptions and handles it in the application specific manner.
     *
     * Fatal exception handling can be intercepted with [CoroutineExceptionHandler] element in the context of a failed coroutine, but such
...
/**
 * This exception holds an exception raised in [CoroutineDispatcher.dispatch] method. When dispatcher methods fail unexpectedly, it is
 * likely a user-induced programmatic bug, such as calling `executor.close()` prematurely. To avoid reporting such exceptions as fatal
 * errors, we handle them with a separate code path. See also #4091.
 *
 * @see safeDispatch
 */
