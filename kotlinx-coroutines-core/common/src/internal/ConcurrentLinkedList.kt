...
/**
 * Returns the first segment `s` with `s.id >= id` or `CLOSED` if all the segments in this linked list have lower `id`, and the list is
 * closed for further segment additions.
 */
internal fun <S : Segment<S>> S.findSegmentInternal(
    ...
    */
    ...
    while (cur.id < id || cur.isRemoved) {
        val next = cur.nextOrIfClosed {
            return SegmentOrClosed(CLOSED)
        }
...
/**
 * Tries to find a segment with the specified [id] following by next references from the [startFrom] segment and creating new ones if
 * needed. The typical use-case is reading this `AtomicRef` values, doing some synchronization, and invoking this function to find the
 * required segment and update the pointer. At the same time, [Segment.cleanPrev] should also be invoked if the previous segments are no
 * longer needed (e.g., queues should use it in dequeue operations).
 *
 * Since segments can be removed from the list, or it can be closed for further segment additions. Returns the segment `s` with `s.id >= id`
 * or `CLOSED` if all the segments in this linked list have lower `id`, and the list is closed.
 */
...
/** Closes this linked list of nodes by forbidding adding new ones, returns the last node in the list. */
...
internal abstract class ConcurrentLinkedListNode<N : ConcurrentLinkedListNode<N>>(prev: N?) {
    ...
    /**
     * This property indicates whether the current node is logically removed. The expected use-case is removing the node logically (so that
     * [isRemoved] becomes true), and invoking [remove] after that. Note that this implementation relies on the contract that the physical
     * tail cannot be logically removed. Please, do not break this contract; otherwise, memory leaks and unexpected behavior can occur.
     */
    abstract val isRemoved: Boolean

    /**
     * Removes this node physically from this linked list. The node should be logically removed (so [isRemoved] returns `true`) at the point
     * of invocation.
     */
    fun remove() {
    ...
    private val aliveSegmentLeft: N?
        get() {
            var cur = prev
            while (cur !== null && cur.isRemoved) cur = cur._prev.value
            return cur
        }

    private val aliveSegmentRight: N
        get() {
            assert { !isTail } // Should not be invoked on the tail node
            var cur = next!!
            while (cur.isRemoved) cur = cur.next ?: return cur
            return cur
        }
}

/**
 * Each segment in the list has a unique id and is created by the provided to [findSegmentAndMoveForward] method. Essentially, this is a
 * node in the Michael-Scott queue algorithm, but with maintaining [prev] pointer for efficient [remove] implementation.
 *
 * NB: this class cannot be public or leak into user's code as public type as [CancellableContinuationImpl] instance-check it and uses a
...
NotCompleted {
    ...
    /**
     * This function is invoked on continuation cancellation when this segment with the specified [index] are installed as cancellation
     * handler via `SegmentDisposable.disposeOnCancellation(Segment, Int)`.
     *
     * @param index the index under which the sement registered itself in the continuation. Indicies are opaque and arithmetics or numeric
     *   intepretation is not allowed on them, as they may encode additional metadata.
