...
internal open class LockFreeLinkedListHead {
    ...
    inline fun forEach(
        ...
        var index: Int = startAddress?.index ?: 0
        while (segment != null) {
            segment.forEach(forbidBitmask = forbidBitmask, startIndex = index, block = block)
    ...
    fun addLastWithoutModifying(node: LockFreeLinkedListNode, permissionsBitmask: Byte): Address? {
        ...
        val segment = tail.findSegmentAndMoveForward(id = index / SEGMENT_SIZE, tail.value, ::createSegment).segment
...
) : Segment<LockFreeLinkedListSegment>(id = id, prev = prev, pointers = pointers)
{
    ...
    fun tryAdd(node: LockFreeLinkedListNode, permissionsBitmask: Byte, indexInSegment: Int): Boolean {
        ...
        cells[indexInSegment].loop { value ->
            ...
            if (value.forbiddenBits and permissionsBitmask != 0.toByte()) return false
...
@JvmInline
internal value class BrokenForSomeElements private constructor(val forbiddenBits: Byte) {
    companion object {
