...
/**
 * Tries to recover stacktrace for given [exception] and [continuation]. Stacktrace recovery tries to restore [continuation] stack frames
 * using its debug metadata with [CoroutineStackFrame] API and then reflectively instantiate exception of given type with original exception
 * as a cause and sets new stacktrace for wrapping exception. Some frames may be missing due to tail-call elimination.
 *
 ...
 * exception of given type with original exception as a cause. Wrapping exception will have proper stacktrace as it's instantiated in the
 * right context.
 *
 * Works only on JVM with enabled debug-mode.
 */
