...
class SupervisorTest : TestBase() {
    @Test
    fun testSupervisorJob() = runTest(unhandled = listOf(
    ...
    @Test
    fun testSupervisorScope() = runTest(unhandled = listOf(
        { it -> it is TestException1 },
        { it -> it is TestException2 },
    )) {
    ...
    @Test
    fun testSupervisorScopeIsolation() = runTest(unhandled = listOf({ it -> it is TestException2 })) {
        val result = supervisorScope {
            expect(1)
            val job = launch {
                expect(2)
