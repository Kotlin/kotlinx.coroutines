...
class SupervisorTest : TestBase() {
    ...
    fun testSupervisorJob() =
        runTest(
            ...
            val job1 =
                launch(supervisor + CoroutineName("job1")) {
                    expect(2)
            ...
            val job2 =
                launch(supervisor + CoroutineName("job2")) {
                    expect(3)
    ...
    @Test
    fun testSupervisorScopeIsolation() =
        runTest(unhandled = listOf({ it -> it is TestException2 })) {
            val result = supervisorScope {
                expect(1)
                val job = launch {
                    expect(2)
    ...
    @Test
    fun testSupervisorThrowsWithFailingChild() =
        runTest(unhandled = listOf({ e -> e is TestException2 })) {
            try {
                supervisorScope {
                    expect(1)
                    launch {
    ...
    fun testSupervisorJobCancellationException() = runTest {
        ...
        val child =
            launch(job + CoroutineExceptionHandler { _, _ -> expectUnreached() }) {
                expect(1)
                hang {
                    expect(3)
                }
