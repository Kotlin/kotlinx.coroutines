...
class CombineParametersTest : TestBase() {
    ...
    fun testFourParametersTransform() = runTest {
        val flow =
            combineTransform(flowOf("1"), flowOf(2), flowOf("3"), flowOf(null)) { a, b, c, d ->
                emit(a + b + c + d)
            }
        assertEquals("123null", flow.single())
    }
    ...
    fun testFiveParameters() = runTest {
        val flow =
            combine(flowOf("1"), flowOf(2), flowOf("3"), flowOf(4.toByte()), flowOf(null)) { a, b, c, d, e ->
                a + b + c + d + e
            }
    ...
    fun testNonMatchingTypes() = runTest {
        val flow =
            combine(flowOf(1), flowOf("2")) { args: Array<Any?> ->
    ...
    fun testNonMatchingTypesIterable() = runTest {
        val flow =
            combine(listOf(flowOf(1), flowOf("2"))) { args: Array<Any?> ->
                args[0]?.toString() + args[1]?.toString()
            }
        assertEquals("12", flow.single())
    }
    ...
    fun testReified() = runTest {
        val value =
            combine(flowOf(1), flowOf(2)) { args: Array<Int> ->
    ...
    fun testReifiedTransform() = runTest {
        val value =
            combineTransform(flowOf(1), flowOf(2)) { args: Array<Int> ->
                    assertIs<Array<Int>>(args)
                    emit(args[0] + args[1])
                }
                .single()
    ...
    fun testTransformEmptyIterable() = runTest {
        val value =
            combineTransform(emptyList()) { args: Array<Int> ->
                    emit(args[0] + args[1])
                }
                .singleOrNull()
        assertNull(value)
    ...
    fun testTransformEmptyVararg() = runTest {
        val value =
            combineTransform { args: Array<Int> ->
                    emit(args[0] + args[1])
                }
                .singleOrNull()
        assertNull(value)
    ...
    fun testEmptyIterable() = runTest {
        val value =
            combine(emptyList()) { args: Array<Int> ->
                    args[0] + args[1]
                }
                .singleOrNull()
        assertNull(value)
