...
class OnCompletionTest : TestBase() {
    ...
    fun testEmitExample() = runTest {
        val flow = flowOf("a", "b", "c").onCompletion() { emit("Done") }
    ...
    sealed class TestData {
        ...
        data class Done(val e: Throwable?) : TestData() {
            override fun equals(other: Any?): Boolean = other is Done && other.e?.message == e?.message
        }
    }
    ...
    fun testCancelledEmitAllFlow() = runTest {
        ...
        flowOf(1, 2, 3).onCompletion { emitAll(MutableSharedFlow()) }.take(1).collect()
    }
    ...
    fun testCancelledEmitAllChannel() = runTest {
        ...
        flowOf(1, 2, 3).onCompletion { emitAll(Channel()) }.take(1).collect()
    }

    /**
     * Tests that the operators that are used to limit the flow (like [take] and [zip]) faithfully propagate the cancellation exception to
     * the original owner.
     */
    @Test
    ...
    fun testEmittingElementsAfterCancellation() = runTest {
        assertEquals(
            1,
            flowOf(1, 2, 3).take(100).onCompletion { emit(4) }.first(),
        )
    }
}
