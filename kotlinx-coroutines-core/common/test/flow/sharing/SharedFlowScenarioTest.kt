...
/**
 * This test suit for [SharedFlow] has a dense framework that allows to test complex suspend/resume scenarios while keeping the code
 * readable. Each test here is for one specific [SharedFlow] configuration, testing all the various corner cases in its behavior.
 */
class SharedFlowScenarioTest : TestBase() {
    @Test
    fun testReplay1Extra2() = testSharedFlow(MutableSharedFlow<Int>(1, 2)) {
        // total buffer size == 3
        expectReplayOf()
        emitRightNow(1)
        expectReplayOf(1)
        ...
        expectReplayOf(3)
        emitRightNow(4)
        expectReplayOf(4) // no prob - no subscribers
        val a = subscribe("a")
        collect(a, 4)
        emitRightNow(5)
        expectReplayOf(5)
        ...
        collect(a, 7)
        emitResumes(e10)
        expectReplayOf(10) // buffer 8, 9 | 10
        collect(a, 8)
        emitResumes(e11)
        expectReplayOf(11) // buffer 9, 10 | 11
        sharedFlow.resetReplayCache()
        expectReplayOf() // 9, 10 11 | no replay
        collect(a, 9)
        emitResumes(e12)
        expectReplayOf(12)
        collect(a, 10, 11, 12)
        expectReplayOf(12) // buffer empty | 12
        emitRightNow(13)
        expectReplayOf(13)
        emitRightNow(14)
        expectReplayOf(14)
        emitRightNow(15)
        expectReplayOf(15) // buffer 13, 14 | 15
        val e16 = emitSuspends(16)
        val e17 = emitSuspends(17)
        val e18 = emitSuspends(18)
        cancel(e17)
        expectReplayOf(15) // cancel in the middle of three emits; buffer 13, 14 | 15
        collect(a, 13)
        emitResumes(e16)
        expectReplayOf(16) // buffer 14, 15, | 16
        collect(a, 14)
        emitResumes(e18)
        expectReplayOf(18) // buffer 15, 16 | 18
        val e19 = emitSuspends(19)
        val e20 = emitSuspends(20)
        val e21 = emitSuspends(21)
        cancel(e21)
        expectReplayOf(18) // cancel last emit; buffer 15, 16, 18
        collect(a, 15)
        emitResumes(e19)
        expectReplayOf(19) // buffer 16, 18 | 19
        collect(a, 16)
        emitResumes(e20)
        expectReplayOf(20) // buffer 18, 19 | 20
        collect(a, 18, 19, 20)
        expectReplayOf(20) // buffer empty | 20
        emitRightNow(22)
        expectReplayOf(22)
        emitRightNow(23)
        expectReplayOf(23)
        emitRightNow(24)
        expectReplayOf(24) // buffer 22, 23 | 24
        val e25 = emitSuspends(25)
        val e26 = emitSuspends(26)
        val e27 = emitSuspends(27)
        cancel(e25)
        expectReplayOf(24) // cancel first emit, buffer 22, 23 | 24
        sharedFlow.resetReplayCache()
        expectReplayOf() // buffer 22, 23, 24 | no replay
        val b = subscribe("b") // new subscriber
        collect(a, 22)
        emitResumes(e26)
        expectReplayOf(26) // buffer 23, 24 | 26
        collect(b, 26)
        collect(a, 23)
        emitResumes(e27)
        expectReplayOf(27) // buffer 24, 26 | 27
        collect(a, 24, 26, 27) // buffer empty | 27
        emitRightNow(28)
        expectReplayOf(28)
        emitRightNow(29)
        expectReplayOf(29) // buffer 27, 28 | 29
        ...
        emitResumes(e32)
        emitResumes(e33)
        expectReplayOf(33) // buffer 30, 32 | 33
        val c = subscribe("c")
        collect(c, 33) // replays
        cancel(e34)
        collect(a, 30)
        emitResumes(e35)
        expectReplayOf(35) // buffer 32, 33 | 35
        cancel(e37)
        cancel(a)
        emitResumes(e36)
        emitResumes(e38)
        expectReplayOf(38) // buffer 35, 36 | 38
        collect(c, 35)
        emitResumes(e39)
        expectReplayOf(39) // buffer 36, 38 | 39
        collect(c, 36, 38, 39)
        expectReplayOf(39)
        cancel(c)
        expectReplayOf(39) // replay stays
    }

    @Test
    fun testReplay1() = testSharedFlow(MutableSharedFlow<Int>(1)) {
        emitRightNow(0)
        expectReplayOf(0)
        emitRightNow(1)
        expectReplayOf(1)
        ...
        expectReplayOf()
        sharedFlow.resetReplayCache()
        expectReplayOf() // buffer 7 | -- no replay, but still buffered
        val b = subscribe("b")
        collect(a, 7)
        emitResumes(e8)
        expectReplayOf(8)
        ...
        expectReplayOf()
        sharedFlow.resetReplayCache()
        expectReplayOf() // nothing is buffered -- both collectors up to date
        emitRightNow(13)
        expectReplayOf(13)
        ...
        cancel(a)
        emitResumes(e15)
        expectReplayOf(15) // cancelling slow subscriber
        collect(b, 15)
        emitResumes(e16)
        expectReplayOf(16)
        collect(b, 16)
    ...
    @Test
    fun testReplay2Extra2DropOldest() = testSharedFlow<Int>(MutableSharedFlow(2, 2, BufferOverflow.DROP_OLDEST)) {
        emitRightNow(0)
        expectReplayOf(0)
        emitRightNow(1)
        expectReplayOf(0, 1)
        ...
        expectReplayOf(5, 6)
        emitRightNow(7)
        expectReplayOf(6, 7) // buffer 4, 5 | 6, 7
        emitRightNow(8)
        expectReplayOf(7, 8) // buffer 5, 6 | 7, 8
        emitRightNow(9)
        expectReplayOf(8, 9) // buffer 6, 7 | 8, 9
        collect(a, 6, 7)
        val b = subscribe("b")
        collect(b, 8, 9) // buffer | 8, 9
        emitRightNow(10)
        expectReplayOf(9, 10) // buffer 8 | 9, 10
        collect(a, 8, 9, 10) // buffer | 9, 10, note "b" had not collected 10 yet
        emitRightNow(11)
        expectReplayOf(10, 11) // buffer | 10, 11
        emitRightNow(12)
        expectReplayOf(11, 12) // buffer 10 | 11, 12
        emitRightNow(13)
        expectReplayOf(12, 13) // buffer 10, 11 | 12, 13
        emitRightNow(14)
        expectReplayOf(13, 14) // buffer 11, 12 | 13, 14, "b" missed 10
        collect(b, 11, 12, 13, 14)
        sharedFlow.resetReplayCache()
        expectReplayOf() // buffer 11, 12, 13, 14 |
        sharedFlow.resetReplayCache()
        expectReplayOf()
    ...
    fun testResumeFastSubscriberOnResumedEmitter() = testSharedFlow<Int>(MutableSharedFlow(1)) {
    ...
    @Test
    fun testSuspendedConcurrentEmitAndCancelSubscriberReplay1() = testSharedFlow<Int>(MutableSharedFlow(1)) {
        val a = subscribe("a")
        emitRightNow(0)
        expectReplayOf(0)
        collect(a, 0)
    ...
    @Test
    fun testSuspendedConcurrentEmitAndCancelSubscriberReplay1ExtraBuffer1() = testSharedFlow<Int>(MutableSharedFlow(
        replay = 1,
