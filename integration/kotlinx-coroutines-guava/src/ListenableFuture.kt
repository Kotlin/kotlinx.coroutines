...
public fun <T> CoroutineScope.future(
    ...
    future.addListener(
      CancellationToCoroutine(future, coroutine),
 ...
 * state when either promise is successfully cancelled.
 *
 * When the returned [Deferred] is cancelled [ListenableFuture.cancel] will be synchronously called
 * on `this` [ListenableFuture]. This will attempt to cancel
 * the `Future`, though cancellation may not succeed and the [ListenableFuture] may complete in a
 * non-cancelled terminal state.
 *
 * This method may receive and suppress the `true` return value from [ListenableFuture.cancel], if
 * the task is cancelled via the [Deferred] reference to it. This is unavoidable, so make sure
 * no idempotent work is performed by any users of the [ListenableFuture] reference to the task
 * if they're the thread that succeeded in cancelling the task - it may not get done.
 *
...
private class CancellationToCoroutine(
    ...
    val coroutineToCancel: AbstractCoroutine<*>
): Runnable {
    override fun run() {
        if (futureToObserve.isCancelled) {
            coroutineToCancel.cancel()
        }
    }
}
 ...
 */
private class OuterFuture<T>(val deferred: Deferred<T>): ListenableFuture<T> {
    val innerFuture = DeferredListenableFuture<T>(deferred)
