...
class PriorityChannelTest : TestBase() {
    @Test
    fun testSimple() = runBlocking {
        val q = PriorityChannel<Int>(1)
    ...
    fun testPriority() = runBlocking {
        ...
        val q = PriorityChannel<Int>(20)
        for (i in 5 downTo 1) {
            expect(idx.incrementAndGet())
            q.send(i)
        }
        expect(6)
        Assert.assertEquals(1, q.receive())
        finish(7)
    }
}