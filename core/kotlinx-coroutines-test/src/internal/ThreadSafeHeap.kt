...
internal class ThreadSafeHeap<T> : SynchronizedObject() where T: ThreadSafeHeapNode, T: Comparable<T> {
    ...
    public inline fun removeFirstIf(predicate: (T) -> Boolean): T? = kotlinx.coroutines.internal.synchronized(this) {
    ...
    // @Synchronized // NOTE! NOTE! NOTE! inline fun cannot be @Synchronized
    public inline fun addLastIf(node: T, cond: () -> Boolean): Boolean = kotlinx.coroutines.internal.synchronized(this) {
        if (cond()) {
            addImpl(node)
            true
        } else {
