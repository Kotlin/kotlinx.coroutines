...
inputFile = "out/results_bfs_channel.csv"
outputFile = "out/bfs-channel.png"
...
def draw(data, plt):
    ...
    for graphName in data.graphName.unique():
        ...
        plt.plot(res_parallel.parallelism, res_parallel.executionTimeAvgMs, label="graphName={}".format(graphName), color=gen_colour, marker=gen_marker)
        res_sequential = data[(data.graphName == graphName) & (data.parallelism == 0)]
        res_sequential = pd.concat([res_sequential.executionTimeAvgMs]*(len(res_parallel.parallelism)))
        plt.plot(res_parallel.parallelism, res_sequential, linestyle='--',label="graphName={}-sequential".format(graphName), color=gen_colour)
