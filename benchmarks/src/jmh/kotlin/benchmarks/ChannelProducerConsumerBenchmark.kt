...
open class ChannelProducerConsumerBenchmark {
    ...
    @Param
    var _1_channel: ChannelCreator = ChannelCreator.RENDEZVOUS
...
//    @Param("1", "2", "4", "8", "16", "32", "64", "128") // Server
    var _4_parallelism: Int = 0

    @Param("10")
    var _5_workSize: Int = 0
    ...
    private suspend fun produce(n: Int) {
        repeat(n) {
            doWork(_5_workSize)
            channel.send(it)
        }
    ...
    private suspend fun consume(n: Int) {
        repeat(n) {
            doWork(_5_workSize)
            channel.receive()
        }
