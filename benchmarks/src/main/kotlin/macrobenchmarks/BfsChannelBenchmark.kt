...
//    DownloadingGraphCreator("USA-DISTANCE", "http://users.diag.uniroma1.it/challenge9/data/USA-road-d/USA-road-d.USA.gr.gz"),
    DownloadingGraphCreator("USA-WEST", "http://users.diag.uniroma1.it/challenge9/data/USA-road-d/USA-road-d.W.gr.gz"),
...
private val PARALLELISM = listOf(1, 2, 4, 8, 16, 32, 64, 128)
/**
...
//    "Kotlin" to { parallelism -> TaskChannelKotlin(parallelism) },
    "Our algorithm NEW" to { parallelism -> TaskChannelNew(parallelism) },
//    "Koval" to { parallelism -> TaskChannelEuropar(parallelism) },
//    "MSQueue" to { parallelism -> TaskChannelMSQueue(parallelism) }
)
/**
    ...
    try {
        for (algo in ALGORITHMS.keys) {
            for (graphCreator in GRAPHS) {
                ...
                writeIterationResults(algo, graphName, 0, result)
                // Execute parallel bfs on different number of coroutines
                for (parallelism in PARALLELISM) {
                    val exitValue = runProcess(
                        ParallelBfsRunner::class.java.name,
                        JVM_OPTIONS,
                        arrayOf(graphName, parallelism.toString(), algo)
                    )
                    if (exitValue != 0) {
...
private fun writeOutputHeader() {
    PrintWriter(OUTPUT).use { writer -> writer.println("algorithm,graphName,parallelism,executionTimeAvgMs,executionTimeStdMs") }
}

private fun writeIterationResults(algorithm: String, graphName: String, parallelism: Int, result: BenchmarkResult) {
    FileOutputStream(OUTPUT, true).bufferedWriter().use { writer ->
        writer.append("$algorithm,$graphName,$parallelism,${String.format(Locale.ROOT, "%.2f",result.executionTime)},${String.format(Locale.ROOT, "%.2f",result.standardDeviation)}\n")
    }
}
...
public class Node(public val id: Int) : Serializable {
    public val neighbours: MutableList<Int> = arrayListOf<Int>()

    public fun addNeighbour(node: Node) {
        neighbours.add(node.id)
    }

    public val distance: AtomicInt = atomic(Int.MAX_VALUE)
}
...
private class TaskChannelKotlin<E>(private val maxWaitingReceivers: Int)
    : LinkedListChannel<E?>(onUndeliveredElement = null)
{
    ...
    override fun onReceiveEnqueued() {
        ...
        if (waitingReceivers >= maxWaitingReceivers) {
            repeat(maxWaitingReceivers) {
                offer(null)
            }
        }
    }
...
@Suppress("INVISIBLE_REFERENCE", "INVISIBLE_MEMBER", "SubscriberImplementation")
private class TaskChannelNew<E>(private val maxWaitingReceivers: Int) : BufferedChannel<E?>(Channel.UNLIMITED) {
    private val waitingReceivers = atomic(0)
...
@Suppress("INVISIBLE_REFERENCE", "INVISIBLE_MEMBER", "SubscriberImplementation")
private class TaskChannelEuropar<E>(private val maxWaitingReceivers: Int) : RendezvousChannelEuropar<E?>() {
    private val waitingReceivers = atomic(0)

    override fun offer(element: E?): Boolean = offerUnlimited(element)
...
@Suppress("INVISIBLE_REFERENCE", "INVISIBLE_MEMBER", "SubscriberImplementation")
private class TaskChannelMSQueue<E>(private val maxWaitingReceivers: Int) : RendezvousChannelMSQueue<E?>() {
    private val waitingReceivers = atomic(0)
...
public interface GraphCacheService : Remote {
    @Throws(RemoteException::class)
    public fun getGraph(graphName: String): List<Node>
}
