 ...
 * [Subscriber.onNext]) values with [send][ProducerScope.send], completes (via [Subscriber.onComplete]) when the coroutine completes or
 * channel is explicitly closed, and emits errors (via [Subscriber.onError]) if the coroutine throws an exception or closes channel with a
 * cause. Unsubscribing cancels the running coroutine.
 *
 * Invocations of [send][ProducerScope.send] are suspended appropriately when subscribers apply back-pressure and to ensure that
 * [onNext][Subscriber.onNext] is not invoked concurrently.
 *
 * Coroutine context can be specified with [context] argument. If the context does not have any dispatcher nor any other
 * [ContinuationInterceptor], then [Dispatchers.Default] is used.
 *
 * **Note: This is an experimental api.** Behaviour of publishers that work as children in a parent scope with respect to cancellation and
...
public fun <T> publish(
    ...
    require(context[Job] === null) {
        "Publisher context cannot contain job in it." + "Its lifecycle should be managed via subscription. Had $context"
    }
...
private val DEFAULT_HANDLER: (Throwable, CoroutineContext) -> Unit = { t, ctx ->
    if (t !is CancellationException) handleCoroutineException(ctx, t)
}
...
public class PublisherCoroutine<in T>(
    ...
    private fun doLockedNext(elem: T): Throwable? {
        ...
        /**
         * This guards against the case when the caller of this function managed to lock the mutex not because some elements were
         * requested--and thus it is permitted to call `onNext`--but because the channel was closed.
         *
         * It may look like there is a race condition here between `isActive` and a concurrent cancellation, but it's okay for a
         * cancellation to happen during `onNext`, as the reactive spec only requires that we *eventually* stop signalling the subscriber.
         */
        if (!isActive) {
            unlockAndCheckCompleted()
        ...
        try {
            ...
            /**
             * The reactive streams spec forbids the subscribers from throwing from [Subscriber.onNext] unless the element is `null`, which
             * we check not to be the case. Therefore, we report this exception to the handler for uncaught exceptions and consider the
             * subscription cancelled, as mandated by https://github.com/reactive-streams/reactive-streams-jvm/blob/v1.0.3/README.md#2.13.
             *
             * Some reactive implementations, like RxJava or Reactor, are known to throw from [Subscriber.onNext] if the execution
             * encounters an exception they consider to be "fatal", like [VirtualMachineError] or [ThreadDeath]. Us using the handler for
             * the undeliverable exceptions to signal "fatal" exceptions is inconsistent with RxJava and Reactor, which attempt to bubble
             * the exception up the call chain as soon as possible. However, we can't do much better here, as simply throwing from all
             * methods indiscriminately would violate the contracts we place on them.
             */
            cancelled = true
    ...
    private fun doLockedSignalCompleted(cause: Throwable?, handled: Boolean) {
        try {
            if (_nRequested.value == SIGNALLED) return
    ...
    override fun request(n: Long) {
        ...
        while (true) { // lock-free loop for nRequested
            ...
            var upd = cur + n
            if (upd < 0 || n == Long.MAX_VALUE) upd = Long.MAX_VALUE
            ...
            if (_nRequested.compareAndSet(cur, upd)) {
                ...
                if (cur == 0L) {
                    /**
                     * In a sense, after a successful CAS, it is this invocation, not the coroutine itself, that owns the lock, given that
                     * `upd` is necessarily strictly positive. Thus, no other operation has the right to lower the value on [_nRequested],
                     * it can only grow or become [CLOSED]. Therefore, it is impossible for any other operations to assume that they own the
                     * lock without actually acquiring it.
                     */
                    unlockAndCheckCompleted()
                }
                return
