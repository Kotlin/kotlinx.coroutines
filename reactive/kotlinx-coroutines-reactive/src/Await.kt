...
/**
 * Awaits the first value from the given publisher without blocking the thread and returns the resulting value, or, if the publisher has
 * produced an error, throws the corresponding exception.
 *
 * This suspending function is cancellable. If the [Job] of the current coroutine is cancelled while the suspending function is waiting,
 * this function immediately cancels its [Subscription] and resumes with [CancellationException].
 *
...
/**
 * Awaits the first value from the given publisher, or returns the [default] value if none is emitted, without blocking the thread, and
...
/**
 * Awaits the first value from the given publisher, or returns `null` if none is emitted, without blocking the thread, and returns the
 * resulting value, or, if this publisher has produced an error, throws the corresponding exception.
 *
 * This suspending function is cancellable. If the [Job] of the current coroutine is cancelled while the suspending function is waiting,
 * this function immediately cancels its [Subscription] and resumes with [CancellationException].
 */
...
/**
 * Awaits the first value from the given publisher, or calls [defaultValue] to get a value if none is emitted, without blocking the thread,
...
/**
 * Awaits the last value from the given publisher without blocking the thread and returns the resulting value, or, if this publisher has
 * produced an error, throws the corresponding exception.
 *
 * This suspending function is cancellable. If the [Job] of the current coroutine is cancelled while the suspending function is waiting,
 * this function immediately cancels its [Subscription] and resumes with [CancellationException].
...
/**
 * Awaits the single value from the given publisher without blocking the thread and returns the resulting value, or, if this publisher has
 * produced an error, throws the corresponding exception.
 *
 * This suspending function is cancellable. If the [Job] of the current coroutine is cancelled while the suspending function is waiting,
 * this function immediately cancels its [Subscription] and resumes with [CancellationException].
...
/**
 * Awaits the single value from the given publisher, or returns the [default] value if none is emitted, without blocking the thread, and
 * returns the resulting value, or, if this publisher has produced an error, throws the corresponding exception.
 *
 * This suspending function is cancellable. If the [Job] of the current coroutine is cancelled while the suspending function is waiting,
 * this function immediately cancels its [Subscription] and resumes with [CancellationException].
 ...
 * ### Deprecation
 *
 * This method is deprecated because the conventions established in Kotlin mandate that an operation with the name `awaitSingleOrDefault`
 * returns the default value instead of throwing in case there is an error; however, this would also mean that this method would return the
 * default value if there are *too many* values. This could be confusing to those who expect this function to validate that there is a
 * single element or none at all emitted, and cases where there are no elements are indistinguishable from those where there are too many,
 * though these cases have different meaning.
 *
 * @throws NoSuchElementException if the publisher does not emit any value
 * @throws IllegalArgumentException if the publisher emits more than one value
 * @suppress
...
/**
 * Awaits the single value from the given publisher without blocking the thread and returns the resulting value, or, if this publisher has
 * produced an error, throws the corresponding exception. If more than one value or none were produced by the publisher, `null` is returned.
 *
 * This suspending function is cancellable. If the [Job] of the current coroutine is cancelled while the suspending function is waiting,
 * this function immediately cancels its [Subscription] and resumes with [CancellationException].
 *
 * ### Deprecation
 *
 * This method is deprecated because the conventions established in Kotlin mandate that an operation with the name `awaitSingleOrNull`
 * returns `null` instead of throwing in case there is an error; however, this would also mean that this method would return `null` if there
 * are *too many* values. This could be confusing to those who expect this function to validate that there is a single element or none at
 * all emitted, and cases where there are no elements are indistinguishable from those where there are too many, though these cases have
 * different meaning.
 *
 * @throws IllegalArgumentException if the publisher emits more than one value
 * @suppress
...
/**
 * Awaits the single value from the given publisher, or calls [defaultValue] to get a value if none is emitted, without blocking the thread,
 * and returns the resulting value, or, if this publisher has produced an error, throws the corresponding exception.
 *
 * This suspending function is cancellable. If the [Job] of the current coroutine is cancelled while the suspending function is waiting,
 * this function immediately cancels its [Subscription] and resumes with [CancellationException].
 ...
 * ### Deprecation
 *
 * This method is deprecated because the conventions established in Kotlin mandate that an operation with the name `awaitSingleOrElse`
 * returns the calculated value instead of throwing in case there is an error; however, this would also mean that this method would return
 * the calculated value if there are *too many* values. This could be confusing to those who expect this function to validate that there is
 * a single element or none at all emitted, and cases where there are no elements are indistinguishable from those where there are too many,
 * though these cases have different meaning.
 *
 * @throws IllegalArgumentException if the publisher emits more than one value
 * @suppress
